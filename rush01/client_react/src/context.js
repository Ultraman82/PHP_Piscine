...
class ProductProvider extends Component {
  ...
  handleCheckout = () => {
    fetch(`http://127.0.0.1:5000/order/${this.state.username}`, {
      method: "POST",
      headers: {
  ...
  gettingCart = () => {
    fetch(`http://127.0.0.1:5000/user/${this.state.username}`)
      .then(response => response.json())
      .then(response => {
  ...
  saveCart = () => {
    ...
    fetch(`http://127.0.0.1:5000/cart/${this.state.username}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
